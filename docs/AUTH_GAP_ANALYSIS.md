...
> produção roda em **Render** (`blaxx-pontos-exe.onrender.com`, deployado do
> **fork** `RVELES/blaxx-pontos` — o canônico `rveles-blaxx/blaxx-pontos` não
> alimenta o serviço) + Neon Postgres. HTTPS é forçado por Render + Netlify;
> logs ficam no painel Render (não `fly logs`).
